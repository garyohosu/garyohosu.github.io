...
### 3. OpenAI、1万エージェントでナビエ=ストークス特異点に迫る

**何が起きた？**

OpenAIはAstra-nextモデルと約1万のエージェントを並列動作させ、88時間・130Bトークンを使って「ナビエ=ストークス方程式の特異点」に迫る計算を実施したと発表しました。総費用は4,000万ドル超。数学の7大難問「ミレニアム懸賞問題」の第2解になりうるとしています。

**ナビエ=ストークス方程式とは？**

流体（水・空気など）の動きを表す方程式です。「特異点（解が発散するポイント）が存在するか否か」は150年以上の未解決問題であり、証明には賞金100万ドルが懸けられています。

**なぜ画期的？**

AIが「既知の知識を整理する」のではなく、**未解決の数学問題に本質的なアプローチ**をしたことが注目されます。多数のエージェントが分業しながら推論する「エージェント群協調」の可能性を示した事例としても重要です。
...
ロイター通信は、AnthropicがIPO（株式公開）前にGPT-6 Astraに対抗する新モデルのリリースを検討していると報じました（[Anthropic News Today September 19, AI Weekly](https://aiweekly.co/ai-news-today/anthropic-news)）。
