...
### 変更内容
- 新記事追加（ainews定期実行）: `_posts/2026-06-19-ai-news-g7-and-ai-power.md`（「AIの最新トレンド5選：G7にAI企業トップが招かれる時代がやってきた」）
- manga画像追加: `assets/img/ai-news-manga-2026-06-19.png`（直近の既存画像 `ai-news-manga-2026-06-16.png` を再利用したプレースホルダー。`OPENAI_API_KEY` 未設定のためDALL-E生成不可）
- 破損していた `ainews.md`（指示プロンプトファイル）を復旧。前回実行（コミット `0766633`）で生成済み記事の本文が誤って上書き保存されており、定期実行の指示が失われていたため、初回コミット（`d75379b`）時点の内容に復元
- `AGENTS.md` にインシデント記録を追加（カテゴリー12: ainews指示ファイルの誤上書き）

### 理由
- 毎日6時のainews定期実行ルーティンにより、直近3日以内のAI関連ニュース（G7サミットでのAI企業トップ参加、米大統領令、Google Antigravity CLI移行、AI企業IPO動向など）を非エンジニア向けに解説する記事を作成
- `ainews.md` の破損を検知したため、定期実行の信頼性確保のために復旧を実施
