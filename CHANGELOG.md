...
### 変更内容
- 新記事追加（ainews定期実行）: `_posts/2026-06-17-ai-news-pause-and-acceleration.md`（「AIの最新トレンド5選：「一旦停止」と「全力加速」が同時に進む2026年6月」）
- manga画像追加: `assets/img/ai-news-manga-2026-06-17.png`（画像生成API未利用のため既存画像をプレースホルダーとして複製）
- ルートの `ainews.md` を修復: 過去の実行で記事本文に上書きされ執筆指示が失われていたため、正しい指示文（コミット`0766633`の親）に復元

### 理由
- 毎日6時の定期実行ルールに基づきAIニュース記事を作成
- `ainews.md` が指示ファイルとして機能していない不具合（過去実行時に記事本文で上書きされていた）を発見したため、本来の指示文に復元し再発防止
