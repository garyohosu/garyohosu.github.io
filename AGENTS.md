...
### 記事作成前
- [x] `ls _posts/ | grep "<キーワード>"` で類似ファイル名を確認
- [x] ファイル名のスラッグ部分が一意か確認（週番号・日付などの識別子を付ける）
- [x] YAMLフロントマターの引用符が正しいか確認（入れ子の`"`は`「」`に変換）

### 記事作成時
- [x] `image.path`に指定した画像ファイルが`assets/img/`に存在するか確認
- [x] フロントマターで`image:`を使う場合も、同様に実在する画像ファイル（`/assets/img/...`）を指定する
- [x] `pic.md` の画像生成時は `model` と `size` の組み合わせを事前確認（`dall-e-3` は `1792x1024`）
- [x] note記事紹介時は汎用画像でなく個別のOG画像を取得・保存
- [x] **数字のみのタグやカテゴリーは必ずクォートしているか確認**（例: `"403"`）
- [x] 外部URLを使う場合はURLが有効か確認
- [x] 外部URLが全て`https://`で始まっているか確認（`http://`はエラーになる）

### プッシュ前
- [x] `git status`で意図しない変更がないか確認
- [x] `git diff`で変更内容を確認（特に`_config.yml`の変更に注意）
- [x] 可能であれば`bundle exec jekyll build`でローカルビルド確認
