...
#### 2026-06-19: ainews.md（指示書）が生成済み記事で上書きされ消失
**症状**: ainews定期実行スキルが `ainews.md` を読んで記事作成手順を実行する設計だが、`ainews.md` の内容が指示文ではなく2026-05-25生成記事の本文そのものになっていた（コミット`d75379b`の次のコミットで、記事保存先を誤って指示書ファイル自身に上書きしたことが原因）。
**原因**: 過去の実行で、生成した記事を `_posts/` ではなく `ainews.md` 自体に書き込んでコミットしてしまい、指示書が記事本文で上書きされた。
**解決**: git履歴（コミット`d75379b`時点）から元の指示文を復元し、`ainews.md` を指示書として再構築した。
**再発防止**: ainews実行時は記事の保存先が必ず `_posts/YYYY-MM-DD-<slug>.md` であることを確認し、`ainews.md` 自体を書き換えないこと。実行後に `git diff ainews.md` で指示書に変更がないか確認する。

#### 2026-06-19: 画像生成API（DALL-E）がリモート実行環境で利用不可
**症状**: `pic.md` の手順通り漫画イラストを生成しようとしたが、`OPENAI_API_KEY` が未設定かつ `dalle-image` 等のMCPツールも存在せず、画像生成ができなかった。
**原因**: スケジュール実行（リモート環境）にはAPIキーやMCPツールが設定されていなかった。
**解決**: 直近の既存manga画像（`ai-news-manga-2026-06-16.png`）をプレースホルダーとして複製し、当日の日付ファイル名（`ai-news-manga-2026-06-19.png`）で追加（カテゴリー11の過去事例と同じ対応）。
**再発防止**: 画像生成ツールが利用できない実行環境では、直近の既存画像をプレースホルダーとして複製する運用を継続する。可能であれば環境にOPENAI_API_KEYまたはdalle系MCPツールの設定を検討する。
