...
- 2026-02-04: 数値タグによるビルドエラーのインシデントと対策を追加
...
**症状**: GitHub Actionsで `Liquid Exception: undefined method 'gsub' for an instance of Integer` エラーが発生しビルド失敗

**原因**:
- 記事のtagsに `403` （数値）を指定した (`tags: [..., 403]`)
- YAMLパーサーがこれを整数として解釈
- Jekyllのテンプレート内で `slugify` フィルタに渡された際に、整数には `gsub` メソッドがないためクラッシュ

**解決**:
- 数値のみのタグをダブルクォートで囲む (`tags: [..., "403"]`)

**再発防止**:
- **数字のみのタグやカテゴリーは必ずクォートする**
