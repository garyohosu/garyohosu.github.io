...
#### 2026-06-17: ainews.md が記事本文で上書きされ指示文が消失
**症状**: ルートの `ainews.md`（ainewsスキルが読む執筆指示ファイル）の内容が、2026-05-25実行時のコミットで生成記事の本文そのものに置き換わっていた。以降の実行でも本来の執筆指示が読めない状態が続いていた可能性がある。
**原因**: 過去の実行で、生成した記事を `_posts/` に保存する代わりに、誤って指示ファイル `ainews.md` 自体に書き込んでしまった。
**解決**: `git show <該当コミットの親>:ainews.md` で元の指示文を復元し、ルートの `ainews.md` に書き戻した。
**再発防止**: ainews実行時は生成記事を必ず `_posts/YYYY-MM-DD-<slug>.md` に保存し、ルートの `ainews.md`／`pic.md` など指示ファイルには絶対に書き込まないことを確認する。実行後は `git diff ainews.md` が空であることをチェックする。
