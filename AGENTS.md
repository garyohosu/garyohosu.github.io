...
### カテゴリー12: ainews指示ファイルの誤上書き
#### 2026-06-19: `ainews.md`（指示プロンプト）が生成済み記事本文で上書きされていた
**症状**: 定期実行（毎日6時）でainewsスキルを実行しようとしたところ、プロジェクトルートの `ainews.md` の内容が、本来の「記事作成指示プロンプト」ではなく、2026-05-25時点で生成済みの記事本文そのものになっていた。
**原因**: 2026-05-25のainews実行（コミット `0766633` "Write AI news roundup"）で、生成した記事を `_posts/` に保存する代わりに、誤って指示ファイル `ainews.md` 自体を生成結果で上書きしてしまった。以降の実行はこの破損した指示ファイルを読む状態になっていた。
**解決**: 初回コミット（`d75379b`）時点の `ainews.md`（正しい指示プロンプト）の内容を `git show` で取得し復元。
**再発防止**: ainews記事生成時は、出力先が必ず `_posts/YYYY-MM-DD-<slug>.md` であることを確認し、`ainews.md` 自体を書き換えない。実行前に `ainews.md` の先頭が指示文（「あなたは、AI分野を専門とするテックジャーナリストです」）で始まっているか確認する。
