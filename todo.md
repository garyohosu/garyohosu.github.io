...
## 2025-11-15 追記（note記事2本追加：初の有料記事宣伝＋ごはん難民AI）by Claude Code

### 1. 初の有料note記事の全力宣伝記事
- **記事追加**: `_posts/2025-11-15-first-paid-note-article-promotion.md`
- **参照**: note「AIに自分のnoteを査定させてみた。Genspark AIブラウザが出した"売れる記事ランキング"が衝撃だった」 https://note.com/hantani/n/n7b252fd4bd98
- **内容構成**:
  - はじめに：人生初の有料note記事であることを強調
  - 記事の内容：Gensparkで過去のnote記事をAI査定、有料化の適正価格と売れる記事ランキングを作成
  - なぜ有料化したのか：実験の手間と恥ずかしい経験を共有する価値
  - どんな人におすすめ：note書いている人、有料化検討中の人、Genspark活用事例に興味がある人
  - 記事の読み方：一部無料・一部有料の構成説明
  - おわりに：全力宣伝の締め、記事リンクを2回配置
- **特徴**:
  - タイトルに「【初の有料note】」と明記して特別感を演出
  - 通常のnote紹介より熱量高めの宣伝色強い文章
  - AIによる記事査定という企画性を強調
  - 行動喚起を複数箇所に配置
- **アイキャッチ画像**: `assets/img/2025-11-15-first-paid-note-article.png`（予定）
- **タグ**: note, 有料記事, Genspark, AIブラウザ, AI査定, 記事分析, コンテンツ評価

### 2. ごはん難民解決プロンプトのnote記事紹介
- **記事追加**: `_posts/2025-11-15-note-gohan-nanmin-ai-prompt.md`
- **参照**: note「知らない駅で『ごはん難民』にならないためのAIプロンプト作ったら、めちゃ便利だった」 https://note.com/hantani/n/ne0fd3a28c4ec
- **内容構成**:
  - 記事タイトル
  - 記事のポイント：
    - 旅先・出張先での「で、どこで食べる？」問題を解決
    - ChatGPT活用の会話型プロンプト開発
    - OpenPromptとカスタムGPTで利用可能
    - AIに丸投げできる仕組み
  - おすすめの読者層：出張・旅行が多い人、レビューサイトが面倒な人、AIプロンプト活用例に興味がある人
  - 関連リンク：OpenPromptとカスタムGPT版
- **特徴**:
  - 共感しやすい「ごはん難民」問題を前面に
  - 実用的なソリューションとしてのプロンプト紹介
  - 複数の利用方法を明示（OpenPrompt/カスタムGPT）
- **アイキャッチ画像**: `assets/img/2025-11-15-note-gohan-nanmin-ai-prompt.png`（予定）
- **タグ**: note, AIプロンプト, ChatGPT, OpenPrompt, カスタムGPT, 飲食店探し, 旅行, 出張
